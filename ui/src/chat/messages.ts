...
function applyAgentEvent(items: ChatItem[], ev: AgentEvent): ChatItem[] {
  switch (ev.type) {
    case "stream_delta": {
      ...
      if (i >= 0) {
        const draft = items[i] as ChatItem & { kind: "agent" };
        const next = items.slice();
        next[i] = { ...draft, text: draft.text + ev.text };
        return next;
      }
      return [...items, { kind: "agent", id: nextId(), text: ev.text, streaming: true }];
    }
    case "assistant_text": {
    ...
    case "result": {
      // Close out any dangling draft, then append the turn footer.
      let next = items;
      const i = findDraft(items);
      if (i >= 0) {
        next = items.slice();
